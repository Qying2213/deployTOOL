...
正式管理后台域名已于 2026-08-31 确认为 `admin.yinlizhangyu.com`。发布工具、安装模板和本机测试已完成；服务器上的独立账号、隔离目录、两个 production helper、最小 sudo 规则及禁用状态的 systemd unit 已安装。**这些准备不代表管理后台已经发布或启动。** DNS、证书、独立数据库账号、独立 COS 权限和正式前端 ZIP 仍须验收；不要把测试 ZIP 当正式包发布。
