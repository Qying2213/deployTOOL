...
sudo -n /usr/local/sbin/loumai-h5-release preflight
```

激活器只接受 `version / preflight / prepare / abort / status / activate / rollback`，并对 release ID、提交、归档与文件哈希、路径、并发锁和当前软链接执行二次校验。
...
本地脚本与服务器激活器当前协议版本为 `2`；任一端还是旧版时，预检会在上传前直接停止。
...
`build` 只生成经过验证的本地版本，不上传。`rollback` 也会保留当前版本的哈希 chunk，避免刚打开的新页面在回滚后加载资源失败。
