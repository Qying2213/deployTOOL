...
test('本地发布器从干净且已同步的精确 Git commit 打包，并避免重复执行质量门禁', () => {
	...
	assert.match(source, /args\.command === 'recover'[\s\S]*\? 'active'/)
...
test('测试服 helper 同步要求显式授权、受控 root 安装和失败恢复', () => {
	...
	assert.match(installer, /RECOVERY_REQUIRED=true/)
	assert.match(installer, /PREFLIGHT_MODE="recover"/)
	assert.match(installer, /preflight active "\$PREFLIGHT_MODE"/)
...
test('测试服受控恢复沿用活动数据库并保持恢复令牌和停写保护', () => {
	const source = readFileSync(join(TOOL_ROOT, 'backend/backend-release.mjs'), 'utf8')
	const helper = remoteHelperContractSource()
	assert.doesNotMatch(source, /recover 只允许用于正式服/)
	assert.doesNotMatch(source, /recover 只用于正式服故障前向修复/)
	assert.doesNotMatch(helper, /recover 只允许用于正式服/)
	assert.match(source, /values\.RECOVERY_TOKEN/)
	assert.match(source, /values\.RECOVERY_PHASE/)
	assert.match(source, /recovery_fail_closed/)
	assert.match(helper, /恢复标记已变化，请重新执行 recover/)
	assert.match(helper, /assert_recovery_units_fail_closed/)
})
...
test('前后端分离配置模板不保存凭据内容，并声明后端仓库、远端与固定运行时', () => {
	...
	assert.match(remoteTemplate, /^LOUMAI_BACKEND_ONESHOT_SERVICES=.*app-push-dispatcher/m)
	assert.match(remoteTemplate, /^LOUMAI_BACKEND_TIMERS=.*app-push-dispatcher\.timer/m)
